...
export async function generateWebsite(input: WebsiteGeneratorInput): Promise<WebsiteGenerationResult> {
  try {
    // Validate templateId
    if (!input.templateId || input.templateId < 1 || input.templateId > 8) {
      throw new Error(`Invalid templateId: ${input.templateId}. Must be between 1 and 8.`)
    }
    ...
    if (!input.businessName || !input.niche || !input.targetAudience) {
      throw new Error('Missing required business information: businessName, niche, and targetAudience are required')
    }
    ...
    if (!systemPrompt || systemPrompt.trim().length === 0) {
      throw new Error('Template prompt file is empty')
    }
...
The HTML should be self-contained and ready to render.
    ...
    try {
      ...
      console.error('[Website Generator] OpenAI API error:', openaiError)
    ...
    let sanitized
    try {
      sanitized = sanitizeAndEnhanceHtml(
        generatedHtml,
        input.templateId,
        input.businessName
      )
    } catch (sanitizeError) {
      console.error('[Website Generator] HTML sanitization error:', sanitizeError)
      throw new Error(`Failed to sanitize generated HTML: ${sanitizeError instanceof Error ? sanitizeError.message : 'Unknown error'}`)
    }
    ...
    let sections
    try {
      sections = buildSectionsFromHtml(sanitized.html)
    } catch (sectionError) {
      console.error('[Website Generator] Section building error:', sectionError)
      throw new Error(`Failed to build sections from HTML: ${sectionError instanceof Error ? sectionError.message : 'Unknown error'}`)
    }

    let layout
    try {
      layout = createDraftLayout({
        dashboardId: input.dashboardId,
        slug: slugify(input.businessName),
        theme: themeTokens,
        sections,
        metadata: {
          title: `${input.businessName} — ${templateConfig.name}`,
      ...
      console.error('[Website Generator] Layout creation error:', layoutError)
      throw new Error(`Failed to create layout: ${layoutError instanceof Error ? layoutError.message : 'Unknown error'}`)
    }
    ...
    console.error('[Website Generator] Error details:', error)
    ...
    if (error instanceof Error) {
      ...
      if (errorMessage.includes('not found') || errorMessage.includes('ENOENT')) {
        throw new Error('Required template prompt file is missing. Please check the file system.')
      }
      
      // Re-throw with original message preserved
      throw new Error(`Website generation failed: ${errorMessage}`)
    }
    
    throw new Error('Website generation failed: Unknown error occurred')
  }
}
