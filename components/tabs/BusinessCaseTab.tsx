...
import { Sparkles, RefreshCw, CheckCircle2, Download, Copy, FileDown } from 'lucide-react'
...
export default function BusinessCaseTab({
  ...
  const handleDownloadMarkdown = () => {
    if (!displayContent) return
    downloadMarkdown(displayContent, 'business-case')
  ...
  const handleExportPDF = async () => {
    if (!displayContent) return
    try {
      await exportAsPDF(displayContent, 'business-case')
  ...
  return (
    <div className="space-y-6">
      ...
      {isGenerating && (
        <GenerationProgress type="business_case" isGenerating={isGenerating} />
      )}
